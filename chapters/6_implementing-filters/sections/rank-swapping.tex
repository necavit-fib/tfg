...
The design of this SDC method follows almost exactly the explanation given in the theoretical background chapter (\sref{Theory:SDCMethods:RankSwapping}). A more formal description of its implementation is given in~\alref{al:rank-swapping} and a more visual explanation is shown in~\fref{fig:rank-swapping-schematic-2}.

\begin{algorithm}[H]
\KwData{$W', A$}
\KwResult{the target instance $\tau$ is anonymized at the end}
\Begin{
	$\tau \leftarrow w_0'$\;
